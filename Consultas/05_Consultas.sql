-- Usa EXISTS para listar los usuarios que sí tienen al menos una reserva registrada.
...
-- Usa NOT EXISTS para listar los vehículos que nunca han tenido un contrato asociado. (Conecta Vehiculo -> Reserva -> Contrato)
...
-- Escribe una subconsulta correlacionada que muestre, por cada vehículo, la fecha de su último evento en Estado_Historial.
...
-- Usa una subconsulta con IN para listar los pagos cuyo id_contrato pertenezca a contratos con sede de devolución distinta a la de recogida.
...
-- Usa ANY o ALL para encontrar las categorías cuya tarifa_dia sea mayor a la de TODAS las categorías que contienen la palabra 'Económico'.
...
-- Construye una subconsulta en el FROM que calcule el total pagado por contrato, y luego únela con Contrato para mostrar el contrato junto a su total.
...
-- Encuentra, para cada usuario, cuál fue su reserva más reciente usando una subconsulta correlacionada con MAX(fecha_inicio).
